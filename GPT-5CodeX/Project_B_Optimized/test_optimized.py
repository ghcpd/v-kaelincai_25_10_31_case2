...
from optimized_code import RegressionAnalyzer
...
def test_normal_case_alpha(records):
    ...
    analyzer.aggregate_scores([records[1]])  # Ensure cache safety works across calls.
...
def test_inputs_are_not_mutated(records):
    analyzer = RegressionAnalyzer()
    snapshot = copy.deepcopy(records)
    analyzer.aggregate_scores([records[1]])
    assert records == snapshot, "sanitisation must not mutate caller data"


def test_large_payload_runs_fast(records):
    analyzer = RegressionAnalyzer()
    batch = records * 200  # Stress cache discipline and recursion.
    start = time.perf_counter()
    analyzer.aggregate_scores(batch)
    elapsed = time.perf_counter() - start
    assert elapsed < 0.75, "optimised path should run within 750ms for synthetic batch"
